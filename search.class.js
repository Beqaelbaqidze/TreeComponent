...
export class SearchClass {
  htmlinput = (
    vIndex
  ) => `<input type="text" class="searchinput${vIndex}" id="searchInput${vIndex}" placeholder="ძებნა..." /><button class="searchBtn${vIndex}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  ...
  htmlULTpl = (
    ...
    return `
      <li class="node${vIndex} nodeTreeLi${vIndex}" data-id="${id}" data-text="${textTitle}">
      <div class="nodeContainerParent${vIndex}" data-id="${id}" data-text="${textTitle}">
    
          <button
            class="parentNode${vIndex} btnTree${vIndex}"
            data-parent-id="${parentId}"
            data-text="${textTitle}"
          >
            <svg
              class="treeArrow${vIndex}"
      ...
      <div class="nodeContainer${vIndex} ${selectedClass}" data-id="${id}" data-text="${textTitle}">
        
        <button class="btnTree${vIndex} nodebtn${vIndex} rotated${vIndex}" data-id="${id}" data-parent-id="${parentId}" style= "width: 0px; height: 0px; position: absolute;"></button>
        ${changeIcons}
        ${icon}
        <p class="nodeText${vIndex}" data-id="${id}">${value}</p>
        </div>
        </div>
        <ul class="children${vIndex} nodeTreeUl${vIndex}" data-id="${id}"></ul>
      </li>
    `;
  };
  ...
  inject(options) {
    ...
    const assembledHTML = `${this.htmlinput(this.index)}`;
    const selector = rootElement;
    container.innerHTML = assembledHTML;
    this.url = options.url;
    const searchInput = document.getElementById(`searchInput${this.index}`);
    const searchBtn = document.querySelector(`.searchBtn${this.index}`);
  ...
  async searchAndAppendNodes(value, vUrl) {
    try {
      ...
      document.getElementById("searchInput" + this.index).style.color = "black";

      if (response.id == -1) {
        document.getElementById("searchInput" + this.index).style.color = "red";
      } else {
        for (let i = 0; i < response.nodes.length; i++) {
          const matchingNodes = this.findNodesById(response.nodes[i].id);
          const targetButton = matchingNodes[0]?.querySelector(
            ".nodebtn" + this.index
          );
          const targetUl = matchingNodes[0]?.querySelector(
            ".children" + this.index
          );
          const targetp = matchingNodes[0]?.querySelector(
          ...
          if (matchingNodes.length > 0) {
            if (targetButton) {
              ...
              targetUl.classList.remove("hidden" + this.index);

              targetButton.classList.add("rotated" + this.index);

              const isSelected = targetp.classList.contains(
                "selected" + this.index
              );
              if (isLastNode) {
                targetp.querySelector(".nodeContainer" + this.index).click();
              }
              if (isSelected) {
            ...
            if (i > 0) {
              ...
              const previousTargetUl = previousNode[0]?.querySelector(
                `.children${this.index}`
              );
              console.log(previousTargetUl, previousNode[0]);
              if (previousTargetUl) {
                let changeIconsHTML = "";
                this.changeIcons.forEach((chIcon, index) => {
                  changeIconsHTML += `<svg class="none${this.index} nodeChangeSvg${this.index} ${chIcon}" data-id="${response.nodes[i].id}" style="width: 18px; margin-right: 8px; height: 18px;" xmlns="http://www.w3.org/2000/svg">
                  <image href="${this.iconsUrl}${response.nodes[i][chIcon]}" class="nodeIcon${this.index} ${chIcon}" data-id="${response.nodes[i].id}" style="width: 18px; height: 18px; cursor: pointer;" />
                </svg>`;
                ...
                this.icons.forEach((icon, index) => {
                  iconsHTML += `<svg class= "${icon} nodeSvg${this.index}" data-id="${response.nodes[i].id}" style="width: 18px; margin-right: 8px;height: 18px; xmlns="http://www.w3.org/2000/svg">
                <image href="${this.iconsUrl}${response.nodes[i][icon]}" class="nodeIcon${this.index} ${icon}" data-id="${response.nodes[i].id}" style="width: 18px; height: 18px; cursor: pointer;" />
                </svg>`;
                  if (index !== this.changeIcons.length - 1) {
                    iconsHTML += ``;
                  }
                ...
                const selectedClass = isLastNode ? "selected" + this.index : "";
                setTimeout(() => {
                  if (selectedClass) {
                    const selectedElement = document.querySelector(
                      `.${selectedClass}`
                ...
                new Promise((resolve, reject) => {
                  ...
                  const vParentNodeBtnEl = document.querySelectorAll(
  ...
  async searchAndClickByParentId(parentId) {
    try {
      ...
      if (matchingNodes.length > 0) {
        const targetButton = matchingNodes[0].querySelectorAll(
          ".nodebtn" + this.index
        );
        ...
        if (targetButton) {
          ...
          if (matchingNodes[11].classList.contains("hidden" + this.index)) {
            //remove it
            matchingNodes[11].classList.remove("hidden" + this.index);
          }
        } else {
          console.error(`Button not found in node with parent ID: ${parentId}`);
        }
        document.querySelector(".searchBtn" + this.index).click();
      } else {
  ...
  async slideButton(event) {
    ...
    if (target.classList.contains("slidebtn" + this.index)) {
      if (
        document
          .querySelector(".customContainer" + this.index)
          .classList.contains("hideTree" + this.index)
      ) {
        document
          .querySelector(".customContainer" + this.index)
          .classList.remove("hideTree" + this.index);
        document
          .querySelector(".customContainer" + this.index)
          .classList.add("showTree" + this.index);
      } else {
        document
          .querySelector(".customContainer" + this.index)
          .classList.add("hideTree" + this.index);
        document
          .querySelector(".customContainer" + this.index)
          .classList.remove("showTree" + this.index);
      }
    }
  }
  // selectAfter() {
